...
public class HttpSink extends AbstractSink implements Configurable {
                ...
                try {
                    ...
                    if (httpStatusCode >= HTTP_STATUS_CONTINUE) {
                        ...
                        boolean shouldRollback =
                                findOverrideValue(httpStatusString, rollbackOverrides, defaultRollback);
                        ...
                        boolean shouldIncrementMetrics =
                                findOverrideValue(httpStatusString, incrementMetricsOverrides, defaultIncrementMetrics);
                        ...
                        if (shouldRollback) {
                            if (shouldBackoff) {
                                ...
                                LOG.info(String.format(
                                        "Got status code %d from HTTP server." + " Rolled back event for retry.",
                                        httpStatusCode));
                            }
                        }
                    } else {
     ...
     */
    private void parseConfigOverrides(
            final String propertyName, final Context context, final Map<String, Boolean> override) {

        Map<String, String> config = context.getSubProperties(propertyName + ".");

        if (config != null) {
            for (Map.Entry<String, String> value : config.entrySet()) {
                LOG.info(String.format(
                        "Read %s value for status code %s as %s", propertyName, value.getKey(), value.getValue()));
     ...
     */
    private boolean findOverrideValue(
            final String statusCode, final HashMap<String, Boolean> overrides, final boolean defaultValue) {
